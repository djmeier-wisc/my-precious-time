...
function getStyles(currSelection, allSelectionList, theme) {
    return {
        fontWeight: allSelectionList.indexOf(currSelection) === -1 ? theme.typography.fontWeightRegular : theme.typography.fontWeightMedium,
    };
}
...
export default function MultiListSelect({busOptions, setCurrBusList, currBusList}) {
    ...
    return (<FormControl className="w-full">
        <InputLabel>Select Route(s)</InputLabel>
        <Select
            ...
            MenuProps={MenuProps}
        >
            {busOptions && busOptions?.map((name) => (<MenuItem
                key={name}
                value={name}
